...
import constants  # store all constants in different file
...
#radii_ext = 8 / 100
covalent_radii = constants.covalent_radii
atomic_weights = constants.atomic_weights
...
def num_to_subnum(number):
    ...
    for digit in str(number):
        utf_digit = utf_sub_dict[digit]
        utf_number = utf_number + utf_digit
    return utf_number
...
def calc_angle(xyzarr: list, i, j, k):
    """
...
# Add new column for weight and covalent radius for each element
# atomic weight dict
...
for groups in grouped:
    summary_bond_table_1.append([groups[1].iloc[0].tolist()[0],
                                 ', '.join(groups[1].sort_values(by=['distance_calc']).distance_calc.apply(
                                     lambda x: '{:.4f}'.format(x)).tolist())])
...
for groups in grouped:
    if len(groups[1]) == 1:
        summary_bond_table_2.append([groups[1].iloc[0].tolist()[0],
                                     groups[1].sort_values(by=['distance_calc']).distance_calc.apply(
                                         lambda x: '{:.4f}'.format(x)).tolist()[0]])
    elif len(groups[1]) == 2:
        summary_bond_table_2.append([groups[1].iloc[0].tolist()[0],
                                     groups[1].sort_values(by=['distance_calc']).distance_calc.apply(
                                         lambda x: '{:.4f}'.format(x)).tolist()[0] +
                                     ' / ' + groups[1].sort_values(by=['distance_calc']).distance_calc.apply(
                                         lambda x: '{:.4f}'.format(x)).tolist()[-1]])
    else:
        summary_bond_table_2.append([groups[1].iloc[0].tolist()[0],
                                     groups[1].sort_values(by=['distance_calc']).distance_calc.apply(
                                         lambda x: '{:.4f}'.format(x)).tolist()[0] +
...
for groups in grouped:
    summary_bond_table_3.append(
        [groups[1].iloc[0].tolist()[0], groups[1].distance_calc.count(), f'{groups[1].distance_calc.mean():.4f}', \
         f'{groups[1].distance_calc.median():.4f}', f'{groups[1].distance_calc.std():.4f}',
         f'{groups[1].distance_calc.std(ddof=0):.4f}', \
...
dist2_df = dist2_df[
    ['atom1_idx', 'cov_radius1', 'atom2_idx', 'cov_radius2', 'distance_calc', 'distance_radii', 'is_bond']]
...
#A0 B1 C2- --> A0-B1-C2
angles_df['A-B-C'] = angles_df['atom1_idx'] + angles_df['fusion_char'] + angles_df['atom2_idx'] + angles_df[
    'fusion_char'] + angles_df['atom3_idx']
#A B C--> A-B-C
angles_df['El1-El2-El3'] = angles_df['element1'] + angles_df['fusion_char'] + angles_df['element2'] + angles_df[
    'fusion_char'] + angles_df['element3']
...
if args.excludeAt:
    sel_angles = angles_df[~angles_df.atom1_idx.isin(args.excludeAt) & ~angles_df.atom2_idx.isin(
        args.excludeAt) & ~angles_df.atom3_idx.isin(args.excludeAt)]

#exclude named elements from input (in Element1 and Element2 and Element3)
if args.excludeEl:
    sel_angles = angles_df[
        ~angles_df.element1.isin(args.excludeEl) & ~angles_df.element2.isin(args.excludeEl) & ~angles_df.element3.isin(
            args.excludeEl)]
...
#sort by elements ascending, A --> Z (not PSE like)
if args.sortascEl:
    sel_angles = sel_angles.sort_values(
        by=['element1', 'element2', 'element3', 'angle_calc', 'atom1_idx', 'atom2_idx', 'atom3_idx'])

#sort by elements descending, A --> Z (not PSE like)
if args.sortdesEl:
    sel_angles = sel_angles.sort_values(
        by=['element1', 'element2', 'element3', 'angle_calc', 'atom1_idx', 'atom2_idx', 'atom3_idx'], ascending=False)
...
for groups in grouped:
    summary_angle_table_1.append([groups[1].iloc[0].tolist()[0],
                                  ', '.join(groups[1].sort_values(by=['angle_calc']).angle_calc.apply(
                                      lambda x: '{:.2f}'.format(x)).tolist())])
...
for groups in grouped:
    if len(groups[1]) == 1:
        summary_angle_table_2.append([groups[1].iloc[0].tolist()[0],
                                      groups[1].sort_values(by=['angle_calc']).angle_calc.apply(
                                          lambda x: '{:.2f}'.format(x)).tolist()[0]])
    elif len(groups[1]) == 2:
        summary_angle_table_2.append([groups[1].iloc[0].tolist()[0],
                                      groups[1].sort_values(by=['angle_calc']).angle_calc.apply(
                                          lambda x: '{:.2f}'.format(x)).tolist()[0] +
                                      ' / ' + groups[1].sort_values(by=['angle_calc']).angle_calc.apply(
                                          lambda x: '{:.2f}'.format(x)).tolist()[-1]])
    else:
        summary_angle_table_2.append([groups[1].iloc[0].tolist()[0],
                                      groups[1].sort_values(by=['angle_calc']).angle_calc.apply(
                                          lambda x: '{:.2f}'.format(x)).tolist()[0] +
...
for groups in grouped:
    summary_angle_table_3.append(
        [groups[1].iloc[0].tolist()[0], groups[1].angle_calc.count(), f'{groups[1].angle_calc.mean():.2f}',
         f'{groups[1].angle_calc.median():.2f}', f'{groups[1].angle_calc.std():.2f}',
         f'{groups[1].angle_calc.std(ddof=0):.2f}',
...
if args.plane1:
    ...
    if ':' in args.plane1:
        ...
        for sep_idx in sep_indices:
            ...
            #get unique index, avoid duplicates from range input and single atom input
...
if args.show or args.showbl or args.shownl:
    ...
    #clumsy but safe
    #for assigning colors to different elements
    ...
    if args.showori:
        arrow_length = sum(abs(i) for i in ax.get_xlim()) + sum(abs(i) for i in ax.get_ylim()) + sum(
            abs(i) for i in ax.get_zlim())
